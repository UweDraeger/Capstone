...
#                   n_max = 10000, 
                   skip_empty_rows = TRUE)
# news_data <- read_lines(paste0(data_dir,"/", nw_file_name), n_max = 10000, skip_empty_rows = TRUE)
...
# bl_words <- bl_text %>% unnest_tokens(word, text) %>% anti_join(stop_words) %>% count(word, sort = TRUE)
# nw_words <- nw_text %>% unnest_tokens(word, text) %>% anti_join(stop_words) %>% count(word, sort = TRUE)
